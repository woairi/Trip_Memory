...
**일상도 여행도, 매일 사진과 함께 느낌·감상·기억에 남는 일**을 남기는 반응형 웹 앱입니다.
...
- 📍 **장소/위치** — 장소명 검색·현재 위치(GPS)·지도 클릭으로 핀 지정(OpenStreetMap), 사진 EXIF 촬영 위치 자동 인식
- 🔍 **검색** — 제목·내용·장소·태그를 한 번에 검색
- 🏷️ **태그** — 기록에 태그를 달고, 태그 칩으로 빠르게 필터링
- 🗺️ **지도** — 위치를 번호 핀(날짜순)과 경로로 표시하고, 나라·도시별로 필터
